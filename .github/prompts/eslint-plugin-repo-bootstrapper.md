...
1. We are adapting an old plugin currently located in the folder provided to this prompt (for example, `eslint-plugin-legacy/`).
2. The root of this repository has been scaffolded using my modern ESLint plugin template. All root `package.json` dependencies & devDependencies are already installed.
...
**Your Instructions:**
5. **Adapt & Migrate:** Move rules, tests, and docs from the provided legacy plugin folder into the template's strict folder structure (`src/`, `docs/`, `test/`).
...
9. **Cleanup:** You may delete files from the provided legacy plugin folder ONLY after you have fully copied, updated, and verified the respective rule/doc.
...
D. The entire project perfectly matches the layout, doc standards, and coding standards of my modern ESLint plugin template.
